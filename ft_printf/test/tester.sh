...
cat tester.log | grep -a --regexp="^ft_printf" | cut -d":" --fields=2,3 > test/difftest_ft_printf.txt
cat tester.log | grep -a --regexp="^printf" | cut -d":" --fields=2,3 > test/difftest_printf.txt
